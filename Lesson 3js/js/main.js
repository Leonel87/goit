...
var elementH = document.querySelector ('.js-hours'),
	elementM = document.querySelector ('.js-minutes'),
    elementS = document.querySelector ('.js-seconds'),
    elementMS = document.querySelector ('.js-miliseconds');

var elementStart = document.querySelector ('.js-start'),
...
function timer () {
    if (switcher == 0) {
		...
		if (mss<10) time[3]='00'+mss;
			...
			if (mss<100) time[3]='0'+mss; 
			else time[3]=mss;
		}
		elementMS.innerHTML=time[3];
		if (sec<10) time[2]='0'+sec; 
		else time[2]=sec; 
		elementS.innerHTML=time[2];
		if (min<10) time[1]='0'+min;
		else time[1]=min;
		elementM.innerHTML=time[1];
		if (hrs<10) time[0]='0'+hrs; 
		else time[0]=hrs;
